...
class GameLogic:
    ...
    def TEST_add_random_targets(self):
        ...
        self.targets.append(Slice_Target(canvas=self.canvas, 
                                         ...
                                         x_speed=random.randint(2, 4) * self.scale_factor,        #random.randint(0.5, 3.5), 
                                         y_pos=1300 * self.scale_factor, 
                                         y_speed=-11.5 * self.scale_factor,        #random.randint(-16,-12), 
                                         gravity=0.05 * self.scale_factor, 
                                         images=target_images))   
        self.targets.append(Slice_Target(canvas=self.canvas, 
                                         x_pos=1920 * self.scale_factor,          #random.randint(600, 900), 
                                         x_speed=-3.5 * self.scale_factor,        #random.randint(-4, -2), 
                                         y_pos=1300 * self.scale_factor, 
                                         y_speed=-11.5 * self.scale_factor,        #random.randint(-16,-12), 
                                         gravity=0.05 * self.scale_factor, 
                                         images=target_images))                                            
